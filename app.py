...
def load_all_data():
    ...
    # 모든 수치 데이터에 대해 쉼표/기호 제거 및 실수형 변환
...
CRIT_RATE_TARGET = float(const.get('Crit_Rate_Max', 0.8)) # 모든 레벨 공통 목표 80%
...
# [보정] 공격력 강화 효율: 기본 공격력의 10% 단위로 조절
atk_step = max(1.0, float(ref['Atk'] * 0.1))
user_atk = st.sidebar.number_input("현재 공격력 (Attack)", value=float(ref['Atk']), step=atk_step)

# [보정] 명중: 요구치까지만 슬라이더 제한
...
# [요청 반영] 용어 명확화 및 슬라이더
user_crit_rate = st.sidebar.slider("치명타 확률 (%)", 0.0, float(CRIT_RATE_TARGET * 100), 0.0) / 100
user_crit_dmg = st.sidebar.slider("치명타 피해 (%)", 150.0, 300.0, 150.0) / 100

# [요청 반영] 설계 표준 목표 가이드 박스 (에러 수정됨)
st.sidebar.divider()
st.sidebar.markdown(f"""
<div style="background-color: #262730; padding: 15px; border-radius: 10px; border: 1px solid #464855;">
    <p style="color: #808495; font-size: 0.8em; margin-bottom: 5px;">🎯 레벨 {selected_lv} 설계 표준 목표</p>
    <span style="font-size: 0.9em;">치명타 확률: <b>{int(CRIT_RATE_TARGET*100)}%</b></span><br>
    <span style="font-size: 0.9em;">치명타 피해: <b>{int(ref['Final_Crit_Dmg']*100)}%</b></span>
</div>
""", unsafe_allow_html=True) # 파라미터명 수정 완료
...
# 지훈 님을 위한 기획 인사이트 요약
st.info(f"**Designer's Insight**: 현재 공격력 {int(user_atk):,} (표준 대비 {round((user_atk/ref['Atk']-1)*100)}% 강화). 명중 {int(ref['Acc']+bonus_acc)} / 요구 {int(ref['Req_Acc'])}")